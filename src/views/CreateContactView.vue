<template>
  <div class="createContact">
    <div class=" flex items-center justify-center p-12">
      <div class="mx-auto w-full max-w-[550px]">
        <form @submit.prevent="handleSubmit">
          <div class="mb-5">
            <label for="name" class="mb-3 block text-base font-medium text-[#07074D]">Full Name</label>
            <input type="text" placeholder="Full Name"
              class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
              v-model="name" required>
          </div>
          <div class="mb-5">
            <label for="email" class="mb-3 block text-base font-medium text-[#07074D]">Email Address</label>
            <input type="email" placeholder="example@domain.com"
              class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
              v-model="email" required>
          </div>
          <div class="mb-5">
            <label for="phone" class="mb-3 block text-base font-medium text-[#07074D]">Phone Number</label>
            <input type="text" placeholder="phone Number"
              class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
              v-model="phone" required>
          </div>
          <div class="mt-6 flex items-center justify-center gap-4">
            <button
              class="transform rounded-md bg-indigo-600/95 px-5 py-3 font-medium text-white transition-colors hover:bg-indigo-700">Add
              Contact</button>
          </div>
        </form>
        <div class="mt-6 flex items-center justify-center gap-4">
          <button @click="reset"
            class="transform rounded-md bg-indigo-600/95 px-5 py-3 font-medium text-white transition-colors hover:bg-indigo-700">Clear</button>
          <router-link to="/"
            class="transform rounded-md bg-indigo-600/95 px-5 py-3 font-medium text-white transition-colors hover:bg-indigo-700">
            Go To Homepage</router-link>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { ref } from 'vue'
import { useStore } from 'vuex'
import { addDoc, collection } from '@firebase/firestore'
import { db } from '../firebase'

export default {
  setup() {
    const store = useStore()
    const name = ref('')
    const email = ref('')
    const phone = ref('')

    const handleSubmit = async () => {
      const contact = {
        name: name.value,
        email: email.value,
        phone: phone.value
      }
      const docRef = await addDoc(collection(db, 'contacts'), contact)
      store.dispatch('addContact', { ...contact, id: docRef.id })
      name.value = ''
      email.value = ''
      phone.value = ''
    }

    const reset = () => {
      name.value = ''
      email.value = ''
      phone.value = ''
    }

    return { name, email, phone, handleSubmit, reset }
  }
}
</script>

